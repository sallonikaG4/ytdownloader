...
def find_ffmpeg():
    ...
    if sys.platform == 'darwin':  # macOS
        bundled_paths = [
            base_path / "ffmpeg" / "bin" / "ffmpeg",
            base_path / "ffmpeg" / "ffmpeg",
            base_path / "Contents" / "Resources" / "ffmpeg" / "bin" / "ffmpeg",
    ...
    elif sys.platform == 'win32':  # Windows (for reference)
        bundled_paths = [
...
def get_default_downloads_dir():
    ...
    macOS: Uses ~/Downloads/YouTube Downloads
    """
    if sys.platform == 'darwin':  # macOS
        downloads_dir = Path(os.path.expanduser('~')) / 'Downloads' / 'YouTube Downloads'
...
class YouTubeDownloader:
    def __init__(self, output_dir=None, bitrate="172k", ffmpeg_path=None):
        ...
        Args:
            output_dir: Directory to save downloaded files (default: user's Downloads/YouTube Downloads)
        ...
        try:
            ...
            raise Exception(
                ...
                f"Please choose a different location or check permissions."
            ) from e
